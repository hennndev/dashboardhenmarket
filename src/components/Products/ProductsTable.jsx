...
const ProductsTable = ({handleOpenModal, handleEdit, handleAddCart, classes, cashier = null, searchTerm = ''}) => {
            ...
            <tbody>
                ...
                {products.map((item, idx) => (
                    <tr className="border-b border-gray-300 cursor-pointer" key={item.id} onClick={(e) => handleClick( item)}>
                        ...
                        <td className={`flex items-center justify-center py-3 ${classes && 'pl-5 w-20 h-20'}`}>
                            <img src={item.productImage} alt={item.productName} className={`${classes ? 'w-full h-full object-cover' : 'w-16 h-16'}`} />
                        </td>
                        ...
                        <td className="text-center px-5">
                            <button className="btn bg-gray-800 text-sm" onClick={(e) => handleOpenModal(item, e)}>
                                Detail
                            </button>
                        </td> 
                        {!cashier && (
